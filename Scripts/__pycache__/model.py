...
"""
Pernicious Anaemia (PA) trainer with:
- Models: xgb, rf, logistic, lightgbm, svm, torch_mlp
- Pipeline w/ ColumnTransformer (impute+scale numeric, OHE categorical)
- RandomizedSearchCV refit on PR-AUC (average precision)
- Probability calibration (isotonic) for non-torch models
- Threshold tuning (optionally enforce minimum precision)
- Metrics + ROC/PR/Confusion/Calibration/Learning-curve plots
- Subgroup metrics
- SHAP summary for tree models (XGB/RF/LightGBM)
- NEW: Grid-search report cards: save cv_results_ to CSV and
       multi-panel plots of PR-AUC vs each hyperparameter per model

Usage examples:
  python pa_trainer_robust.py --data data.csv --model xgb --min_precision 0.8 --output_dir out_xgb
  python pa_trainer_robust.py --data data.csv --model lightgbm --output_dir out_lgbm
  python pa_trainer_robust.py --data data.csv --model svm --output_dir out_svm
  python pa_trainer_robust.py --data data.csv --model torch_mlp --output_dir out_torch
"""
...
class TorchMLPClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.epochs):
            ...
            self.model.eval()
            with torch.no_grad():
                vloss = 0.0; n_batches = 0
                for xb, yb in dl_val:
                    xb, yb = xb.to(dev), yb.to(dev)
                    vloss += criterion(self.model(xb), yb).item(); n_batches += 1
...
class Trainer:
    ...
    def _make_base(self):
        ...
        if m == "svm":
            # We'll wrap with calibration to get probabilities
    ...
    def train(self):
        ...
        if self.model_type.lower() != "torch_mlp":
            ...
            clf = base  # torch model outputs sigmoid probs
        ...
        # NEW: save CSV + multi-paneled plots of PR-AUC vs hyperparams
        self._save_gridsearch_csv_and_plots(search, self.model_type)
    ...
    # -------- Grid-search report cards --------
    def _save_gridsearch_csv_and_plots(self, search, model_name: str):
        """Save cv_results_ and plot PR-AUC vs each hyperparameter in a multi-paneled figure."""
        results = search.cv_results_
        out_csv = os.path.join(self.output_dir, f"gridsearch_{model_name}.csv")
        pd.DataFrame(results).to_csv(out_csv, index=False)
        print(f"📑 Grid search results saved: {out_csv}")

        # Pick PR-AUC column
        score_key = None
        for k in results.keys():
            if k.startswith("mean_test_") and ("pr_auc" in k or "average_precision" in k):
                score_key = k; break
        if score_key is None:
            for k in results.keys():
                if k.startswith("mean_test_") and "roc_auc" in k:
                    score_key = k; break
        if score_key is None:
            print("⚠️ No suitable mean_test_* score found for plotting. Skipping plots.")
            return

        # std column (if present)
        std_key = f"std_test_{score_key[len('mean_test_'):]}"
        param_cols = [c for c in results.keys() if c.startswith("param_")]
        if not param_cols:
            print("⚠️ No param_* columns in cv_results_. Skipping plots.")
            return

        # Tidy dataframe
        cols_to_take = param_cols + [score_key]
        if std_key in results:
            cols_to_take.append(std_key)
        df = pd.DataFrame({c: results[c] for c in cols_to_take})
        df.rename(columns={score_key: "mean_test_pr_auc"}, inplace=True)
        if std_key in df.columns:
            df["std_test"] = df[std_key]
        else:
            split_cols = [c for c in results.keys() if c.startswith("split") and c.endswith("_test_score")]
            if split_cols:
                temp = pd.DataFrame({c: results[c] for c in split_cols})
                df["std_test"] = temp.std(axis=1)
            else:
                df["std_test"] = 0.0

        # Convert numeric-like params
        for p in param_cols:
            try:
        ...
        # Build plots per hyperparameter
        plots = []
        for p in param_cols:
            grp = df.groupby(p, dropna=False, observed=True)["mean_test_pr_auc"].agg(["mean", "std", "count"]).reset_index()
            if pd.api.types.is_numeric_dtype(grp[p]):
        ...
        fig = plt.figure(figsize=(6*ncols, 3.8*nrows))
        for i, (p, grp) in enumerate(plots, 1):
            ax = fig.add_subplot(nrows, ncols, i)
            x = grp[p]; y = grp["mean"]; yerr = grp["std"].fillna(0.0)

            if pd.api.types.is_numeric_dtype(x):
                ax.errorbar(x, y, yerr=yerr, fmt="-o", capsize=3)
                try:
                    xmin, xmax = np.nanmin(x.values.astype(float)), np.nanmax(x.values.astype(float))
                    if xmin > 0 and xmax / max(xmin, 1e-12) >= 100:
                        ax.set_xscale("log")
                except Exception:
                    pass
            else:
                coords = np.arange(len(x))
                ax.errorbar(coords, y, yerr=yerr, fmt="o-", capsize=3)
                ax.set_xticks(coords)
                ax.set_xticklabels([str(v) for v in x], rotation=30, ha="right")

            ax.set_title(p.replace("param_", ""), fontsize=11)
            ax.set_ylabel("Mean test PR-AUC")
            ax.grid(alpha=0.25)

        fig.suptitle(f"Search – {model_name} (PR-AUC vs hyperparameters)", fontsize=14, y=1.02)
        fig.tight_layout()
        if self.savefigs:
            path = os.path.join(self.fig_dir, f"gridsearch_{model_name}.png")
            fig.savefig(path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            print(f"🖼️ Grid-search plot saved: {path}")
        else:
            plt.show()
...
# ---------- CLI ----------
def parse_args():
    p = argparse.ArgumentParser(description="Robust PA trainer with multiple models and grid-search report cards.")
